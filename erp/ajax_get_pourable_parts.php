...
foreach ($all_parts as $part) {
    ...
    // Get already poured quantity for this PO and part (sum of pouring_batches.quantity)
    $stmt_proc = $pdo->prepare("SELECT COALESCE(SUM(quantity), 0) FROM pouring_batches WHERE po_id = ? AND product_id = ?");
    $stmt_proc->execute([$po_id, $product_id]);
    $already_poured = (float)($stmt_proc->fetchColumn() ?: 0);
    if ($already_poured < $required_qty - 0.001) {
        $part['completed_qty'] = $already_poured;
        $parts[] = $part;
    }
}
