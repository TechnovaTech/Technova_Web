...
foreach ($all_parts as $part) {
    ...
    // Get already processed quantity for this PO and part (sum of shot_blasting_batches.pieces_processed)
    $stmt_proc = $pdo->prepare("SELECT COALESCE(SUM(pieces_processed), 0) FROM shot_blasting_batches WHERE po_id = ? AND product_id = ?");
