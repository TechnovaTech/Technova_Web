...
foreach ($all_parts as $part) {
    ...
    // Get already processed quantity for this PO and part (sum of fettling.output_quantity)
    $stmt_proc = $pdo->prepare("SELECT COALESCE(SUM(output_quantity), 0) FROM fettling WHERE po_id = ? AND product_id = ?");
